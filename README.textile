...
Handlers in this library are considered consumers of a particular class of events. Handlers can work on the thread that sent the event or you can instantiate a MainThreadEventHandler that uses the main thread to execute operations in response to events.
